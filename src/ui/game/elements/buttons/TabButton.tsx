...
const StyledTabButton = styled.button<{
    ...
    justify-content: ${(props) =>
        props.hasLabel && props.hasIcon
            ? "flex-start"
            : "center"}; /* Center if icon-only or label-only */
    ...
    color: ${(props) =>
        props.isActive ? props.activeTextColor : props.textColor};
    ...
    &:hover:not(:disabled):not(.isActive) {
        background-color: ${(props) =>
            props.isActive ? props.activeBgColor : "rgba(255, 255, 255, 0.1)"};
    ...
    .tab-icon-element {
        ...
        margin-right: ${(props) =>
            props.hasLabel ? "8px" : "0"}; /* Space only if label exists */
    }
`;
...
export const TabButton: React.FC<TabButtonProps> = ({
    ...
    if (!label && !iconSrc) {
        console.warn(
            "TabButton: Either 'label' or 'iconSrc' must be provided.",
        );
        return null;
    }
