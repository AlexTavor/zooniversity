...
export const TreePanelContentCollapsed: React.FC<
    TreePanelContentCollapsedProps
> = ({ data }) => {
    const { harvestableInfo, foragableInfo } = data;

    const primaryWoodYield = harvestableInfo?.yield.find(
        (drop) => drop.type === ResourceType.WOOD,
    );
    const woodResourceDetails = primaryWoodYield
        ? ResourceConfig[primaryWoodYield.type as ResourceType]
        : null;
    const foodResourceDetails = foragableInfo
        ? ResourceConfig[foragableInfo.resourceType]
        : null;
    ...
    return (
        <CollapsedWrapper>
            ...
            {harvestableInfo && (
                <CollapsedSection
                    title={`Health: ${harvestableInfo.hitpointsCurrent}/${harvestableInfo.hitpointsMax}`}
                >
                    {harvestableInfo.isSelectedForCutting && (
                        <Icon
                            iconSrc={markedForCuttingIconSrc}
                            shape="square"
                            size="16px"
                            alt="Marked for Cutting"
                    ...
                    {primaryWoodYield && woodResourceDetails && (
                        <>
                            <Icon
                                iconSrc={woodResourceDetails.icon}
                                shape="square"
                                size="16px"
                                alt={primaryWoodYield.type}
                                backgroundColor="transparent"
                                borderColor="transparent"
                            />
            ...
            {foragableInfo && foodResourceDetails && (
                <CollapsedSection
                    title={`Forageable: ${Math.floor(foragableInfo.currentAmount)}/${foragableInfo.maxAmount}`}
                >
                    <Icon
                        iconSrc={foodResourceDetails.icon}
                        shape="square"
                        size="16px"
                        alt={foragableInfo.resourceType}
                        backgroundColor="transparent"
                        borderColor="transparent"
                    />
                    <StyledFillbar
