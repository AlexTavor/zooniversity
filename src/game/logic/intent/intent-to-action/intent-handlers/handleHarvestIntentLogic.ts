    ...
    if (nearbyTreeIds && nearbyTreeIds.length > 0) {
        for (const treeId of nearbyTreeIds) {
            ...
            if (isTreeValidForHarvest(ecs, treeId)) {
                ...
                if (!slots || !transform) return false; // No interaction slots or transform, cannot harvest
                ...
                const harvesting =
                    ecs.getComponent(characterEntity, HarvestingState) ??
                    new HarvestingState(treeId, targetPos);
                harvesting.target = treeId;
                harvesting.targetPos = targetPos;

                !ecs.hasComponent(characterEntity, HarvestingState) &&
                    ecs.addComponent(characterEntity, harvesting);
...
export function canHarvest(ecs: ECS, entity: Entity): boolean {
    const allTrees = ecs.getEntitiesWithComponents([
        Tree,
...
function updateActions(
    ...
    if (!locomotion.arrived) {
        ...
        actionIntent.currentPerformedAction !== CharacterAction.CHOPPING &&
        actionIntent.actionData?.targetTreeEntityId !== treeId
    ) {
