...
function setWalkingToStrollPoint(
...
function findValidStrollTrees(
    ecs: ECS,
    excludeTreeId?: Entity,
): { id: Entity; transform: Transform }[] {
    ...
    for (const treeId of treeEntities) {
        ...
        validTrees.push({
            id: treeId,
            transform: ecs.getComponent(treeId, Transform),
        });
    }
    return validTrees;
}
...
function selectNextStrollTarget(
    ...
    const candidates = availableTrees
        .map((t) => ({
            id: t.id,
            transform: t.transform,
            distSq:
                MathUtils.distance(
                    currentPosOrReferenceTransform,
                    t.transform,
                ) ** 2,
        }))
    ...
    const selectionPool = candidates.slice(
        0,
        MAX_CANDIDATE_TREES_FOR_RANDOM_STROLL,
    );
    const chosenTreeInfo =
        selectionPool[Math.floor(Math.random() * selectionPool.length)];

    const strollTargetPos = calculateBehindTreePosition(
        chosenTreeInfo.transform,
        referenceTransformForOrientation,
    );
    return { treeId: chosenTreeInfo.id, strollTargetPos };
}

function calculateBehindTreePosition(treePos: Pos, referencePos: Pos): Pos {
    const dirVector = MathUtils.normalize(
        MathUtils.subtract(treePos, referencePos),
    );
    ...
    return MathUtils.add(
        treePos,
        MathUtils.multiply(dirVector, STROLL_BEHIND_TREE_OFFSET),
    );
}
...
export function handleRestIntentLogic(
    ...
    if (strollComp.isPausedAtTarget) {
        if (currentGameTime >= strollComp.pauseUntilTime) {
            ...
            if (strollComp.currentTargetTreeId) {
                setStrollingAtPoint(
                    actionIntent,
                    strollComp.currentTargetTreeId,
                );
            } else {
    ...
    if (!strollComp.currentPathTargetPos || !strollComp.currentTargetTreeId) {
        const referenceTransform = ecs.getComponent(
            strollComp.referencePointEntityId,
            Transform,
        );
        ...
        const lastTreePosSource = strollComp.lastTargetTreeId
            ? ecs.getComponent(strollComp.lastTargetTreeId, Transform)
            : referenceTransform;
        const nextTargetInfo = selectNextStrollTarget(
            ecs,
            lastTreePosSource,
            referenceTransform,
            strollComp.lastTargetTreeId,
        );
    ...
    if (!locomotion.arrived) {
        ...
        if (
            ...
            currentWalkData.ultimateTargetEntityId !==
                strollComp.currentTargetTreeId
        ) {
            setWalkingToStrollPoint(
                actionIntent,
                strollComp.currentPathTargetPos!,
                strollComp.currentTargetTreeId,
            );
        }
    ...
    if (actionIntent.currentPerformedAction != CharacterAction.STROLLING) {
        ...
        const pauseDuration =
            Math.random() *
                (MAX_PAUSE_DURATION_GAME_MINUTES -
                    MIN_PAUSE_DURATION_GAME_MINUTES) +
            MIN_PAUSE_DURATION_GAME_MINUTES;
