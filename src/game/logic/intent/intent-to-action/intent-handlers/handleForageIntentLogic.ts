...
function setWalkingToForageSlot(
...
function getPrimaryForagingReferencePoint(
    ecs: ECS,
    ...
    if (
        workerInfo &&
        workerInfo.dojoId !== null &&
        ecs.hasEntity(workerInfo.dojoId)
    ) {
        if (ecs.hasComponent(workerInfo.dojoId, WoodDojo)) {
            // Replace WoodDojo with actual KitchenComponent if it's distinct
    ...
    if (
        ...
        ecs.hasEntity(home.homeEntityId)
    ) {
...
export function canForage(ecs: ECS, characterEntity: Entity): boolean {
    const referenceEntityId = getPrimaryForagingReferencePoint(
        ecs,
        characterEntity,
    );
    ...
    if (nearbyEntityIds && nearbyEntityIds.length > 0) {
        for (const entityId of nearbyEntityIds) {
            ...
            const foragableComponent = ecs.getComponent(
                entityId,
                ForagableComponent,
            );
            ...
            if (
                foragableComponent &&
                foragableComponent.currentAmount ==
                    foragableComponent.maxAmount &&
                slots
            ) {
                // Check if ANY forage slot is available or already occupied by this character
                if (
                    slots
                        .getSlotsArray(SlotType.FORAGE)
                        .some(
                            (slot) =>
                                slot.occupiedBy === null ||
                                slot.occupiedBy === characterEntity,
                        )
                ) {
...
function findAndAssignForageTarget(ecs: ECS, characterEntity: Entity): boolean {
    ...
    if (nearbyEntityIds && nearbyEntityIds.length > 0) {
        for (const entityId of nearbyEntityIds) {
            ...
            const foragableComp = ecs.getComponent(
                entityId,
                ForagableComponent,
            );
            ...
            if (
                foragableComp &&
                foragableComp.currentAmount > 0.2 &&
                targetTransform &&
                slots
            ) {
                ...
                if (offset) {
                    ...
                    let foragingState = ecs.getComponent(
                        characterEntity,
                        ForagingState,
                    );
                    if (!foragingState) {
                        foragingState = new ForagingState(
                            entityId,
                            targetPosition,
                        );
...
function updateForageActions(
    ...
    const {
        targetForagableEntityId: targetId,
    ...
    if (
        !ecs.hasEntity(targetId) ||
        (ecs.getComponent(targetId, ForagableComponent)?.currentAmount || 0) <=
            0
    ) {
    ...
    if (
        roundedCharX !== exactTargetPosition.x ||
        roundedCharY !== exactTargetPosition.y
    ) {
...
export function handleForageIntentLogic(
    ...
    if (!foragingState || foragingState.targetForagableEntityId === -1) {
        ...
        if (
            newlyAssignedState &&
            newlyAssignedState.targetForagableEntityId !== -1
        ) {
            updateForageActions(ecs, entity, actionIntent, newlyAssignedState);
            turnToTarget(
                ecs,
                entity,
                newlyAssignedState.targetForagableEntityId,
            );
        }
        return;
    }

    const forageableComponent = ecs.getComponent(
        foragingState.targetForagableEntityId,
        ForagableComponent,
    );
