...
export class StatCalculator {
    private static getBaseValue(
        ...
        switch (statType) {
            ...
            case AffectedStat.HARVEST_SPEED: {
                const effectiveWorkSpeed = StatCalculator.getEffectiveStat(
                    ecs,
                    entity,
                    AffectedStat.WORK_SPEED,
                );
                const harvesterForHS = ecs.getComponent(
                    entity,
                    HarvesterComponent,
                );
                const baseHarvestSpeedAdditive =
                    harvesterForHS?.harvestPerMinute ?? 0;
                return effectiveWorkSpeed + baseHarvestSpeedAdditive;
            }
            case AffectedStat.FORAGING_SPEED: {
                // Scale it down to foraging numbers
                const effectiveWorkSpeed =
            ...
            default:
                console.warn(
                    `StatCalculator: Base value for stat type ${statType} not defined.`,
                );
                return 0;
        }
    }
    ...
    public static getEffectiveStat(
        ...
        relevantEffects.sort((a, b) => {
            const typeOrderA =
                a.type === BuffEffectApplicationType.FLAT_ADDITIVE ? 1 : 2;
            const typeOrderB =
                b.type === BuffEffectApplicationType.FLAT_ADDITIVE ? 1 : 2;
        ...
        for (const effect of relevantEffects) {
            if (
                effect.type === BuffEffectApplicationType.PERCENT_MULTIPLICATIVE
            ) {
