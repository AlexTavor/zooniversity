...
export class WeatherSystem extends System {
    ...
    update(entities: Set<Entity>, delta: number): void {
        ...
        if (entities.size > 1) {
            console.warn(
                "WeatherSystem is not designed to handle multiple entities.",
            );
        }
    ...
    updateWeather(entity: Entity, delta: number) {
        ...
        weather.windDirection =
            dirNoise > WeatherConfig.Wind.DirectionThreshold ? 1 : -1;

        // Cloud Cover (Inverted Bell)
        const t2 = this.time * WeatherConfig.CloudCover.Frequency;
        const coverNoise = this.noise.noise(
            t2,
            WeatherConfig.CloudCover.NoiseOffset,
        );
