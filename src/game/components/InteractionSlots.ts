...
export class InteractionSlots {
    ...
    constructor(definitions: Partial<Record<SlotType, SlotDefinition>>) {
        for (const [type, def] of Object.entries(definitions) as [
            SlotType,
            SlotDefinition,
        ][]) {
            if (def) {
                // Ensure def is not undefined if Record value can be optional
    ...
    public reserve(
        entity: Entity,
        slotType: SlotType = SlotType.WORK,
    ...
    public inUse(type: SlotType): boolean {
        if (this.slots.has(type)) {
            return this.slots
                .get(type)!
                .some((slot) => slot.occupiedBy !== null);
        }
        return false;
    }
}
