...
import { ValueRef } from './ProgressBar'; // Assuming ValueRef is in ProgressBar.ts or a common types file
...
export class ForagableEffect implements EffectInstance {
  ...
  public start(): void {
    if (this.sprites.length > 0) { // Already initialized
    ...
    for (let i = 0; i < this.config.maxSprites; i++) {
        ...
        const appleSprite = scene.add.sprite(pos.x, pos.y, "plants", "foragable_item");
