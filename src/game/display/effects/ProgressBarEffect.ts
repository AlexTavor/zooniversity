...
export class ProgressBarEffect {
    ...
    private config: Required<Omit<ProgressBarConfig, "border">> & {
        border?: BorderConfig;
    };
    ...
    private drawForeground(
        ...
        if (this.config.direction === "horizontal") {
            ...
            this.drawRoundedRect(
                this.anchorPos.x,
                this.anchorPos.y + (barLength - filled),
                thickness,
                filled,
                0,
            );
    ...
    public update(_delta: number, position: Pos): void {
        ...
        const { minWidth, maxWidth, minWidthValue, maxWidthValue } =
            this.config.size;

        // REMOVED localScale multiplication from these lines:
        const thickness = 6;
        const barLength = this.calculateWidth(
            max,
            minWidthValue,
            maxWidthValue,
            minWidth,
    ...
    private updateDisplaySize(): void {
        const sprite = this.container.list.find(
            (obj) => obj instanceof Phaser.GameObjects.Sprite,
        ) as Phaser.GameObjects.Sprite;
        if (sprite) {
            this.displayWidth = sprite.displayWidth;
